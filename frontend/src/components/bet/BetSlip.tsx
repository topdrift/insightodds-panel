...
export default memo(function BetSlip({ eventId, selection, onClose, onPlaced }: BetSlipProps) {
  ...
  useEffect(() => {
    if (selection) {
      setOdds(selection.price.toString());
      setAmount(0);
      setError(null);
      setSuccessAnim(false);
      // Focus amount input after brief delay for animation
  ...
  const profit = amount > 0 && parsedOdds > 1 ? amount * (parsedOdds - 1) : 0;
  const loss = amount;
  const balance = user?.balance || 0;
  const exposure = user?.exposure || 0;
  const insufficientBalance = isBack ? amount > balance : profit > balance;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const isBackBet = selection.type === 'BACK';
      await api.post('/bet/place', {
        eventId,
        marketId: selection.marketId,
        selectionId: selection.selectionId,
        runnerName: selection.runnerName,
        marketName: selection.marketName || 'Match Odds',
        amount,
        ...(isBackBet
          ? { back: parsedOdds, backRate: parsedOdds }
          : { lay: parsedOdds, layRate: parsedOdds }),
        profit: isBackBet ? amount * (parsedOdds - 1) : amount,
        loss: isBackBet ? amount : amount * (parsedOdds - 1),
      });
  ...
  }, [selection, parsedOdds, amount, insufficientBalance, eventId, onPlaced]);

  // Handle Enter key
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && !loading && amount > 0 && parsedOdds > 1) {
      handleSubmit();
    }
    if (e.key === 'Escape') {
      onClose();
    }
  }, [handleSubmit, onClose, loading, amount, parsedOdds]);
  ...
  return (
    <div
      ...
      className={cn(
        'rounded-2xl border overflow-hidden shadow-glass-lg animate-slide-up',
        isBack
    ...
    >
      ...
      {selection.marketName && (
        <div className="px-4 pt-1">
          <span className="text-[10px] text-gray-500">{selection.marketName}</span>
        </div>
      )}
      ...
      <div className="px-4 pt-3 pb-2 space-y-3">
        ...
        {amount > 0 && parsedOdds > 1 && (
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="rounded-lg bg-loss-surface px-3 py-2 text-center">
              ...
              <span className={cn(
                'text-sm font-bold',
                isBack ? 'text-loss' : 'text-profit'
              )}>
                &#8377;{formatIndian(isBack ? loss : amount)}
              </span>
            </div>
          </div>
        )}
        ...
        <button
          ...
          disabled={loading || amount <= 0 || parsedOdds <= 1 || insufficientBalance}
          className={cn(
